...
import { InternalServerError, InvalidAPIKeyError, NetworkError, ServiceUnavailableError } from '../../src/errors';
...
describe('AsyncHTTPClient', () => {
  ...
  it('rejects http:// baseUrl', () => {
    expect(() => new AsyncHTTPClient({ apiKey: 'rdk-test', baseUrl: 'http://api.rodiumai.io/v1' })).toThrow(
      'HTTPS is required'
    );
  });
  ...
  it('maps 401 to InvalidAPIKeyError', async () => {
    ...
    const client = new AsyncHTTPClient({ apiKey: 'rdk-test', baseUrl: `${API_BASE}/v1`, maxRetries: 0 });

    await expect(client.request({ method: 'POST', path: '/chat/completions' })).rejects.toThrow(InvalidAPIKeyError);
  });
  ...
  it('maps 500 to InternalServerError', async () => {
    ...
    await expect(client.request({ method: 'POST', path: '/chat/completions' })).rejects.toThrow(InternalServerError);
  });
  ...
  it('maps 503 to ServiceUnavailableError', async () => {
    ...
    await expect(client.request({ method: 'POST', path: '/chat/completions' })).rejects.toThrow(ServiceUnavailableError);
  });
  ...
  it('throws NetworkError on transport failure', async () => {
    ...
    await expect(client.request({ method: 'POST', path: '/chat/completions' })).rejects.toThrow(NetworkError);
  });
  ...
  it('extracts backend error message', async () => {
    ...
    await expect(client.request({ method: 'POST', path: '/chat/completions' })).rejects.toThrow('Bad input');
  });
